...
class FileUtils
{
    ...
    protected static function formatArrayConfig($file, $i = 0)
    {
        ...
        if ($file->mimeType == "text/plain") {
            ...
            if ($file->mimeType == "application/pdf" || $file->mimeType == "application/octet-stream" || $file->mimeType == "video/mp4") {
        ...
        $arr['config'] = array(
            "size" => filesize(
                $fpath
            ), "type" => $type, "filetype" => $filetype, "previewAsData" => $previewAsData, "caption" => $file->name, "width" => "120px", "key" => $i, "downloadUrl" => $downloadUrl, "url" => "/bootstrap4/deletefile"
        );
        $arr['url'] = $url;
        $arr['path'] = $fpath;
